...
// 設定
const CONFIG = {
  keyPath: './xxx.json', // 送信元のキーファイル
  csvPath: './wallets.csv', // 送信先アドレスのCSVファイル
  outputPath: './distribution_results.csv', // 結果を保存するCSVファイル
  solPerWallet: 0.1, // 1ウォレットあたりの送金額 (SOL)
  batchSize: 20, // 1トランザクションあたりの最大送金数
  rpcUrl: process.env.RPC_URL || 'https://api.mainnet-beta.solana.com', // デフォルトのRPC URL
...
async function loadWallets(csvPath) {
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(csvPath)
      ...
      .on('data', (row) => {
        // CSV形式に応じて適切なキーを使用
        const address = row.wallet_address || row.address || Object.values(row)[0];
...
// バッチ送金を実行
async function processBatch(connection, keypair, batch, batchNumber, solPerWallet, resultsWriter) {
  ...
  try {
    ...
    for (const address of batch) {
      try {
        ...
        console.error(`アドレス "${address}" はスキップされました: ${error.message}`);
    ...
    if (transaction.instructions.length === 0) {
      console.error('有効なインストラクションがありません。このバッチはスキップされます。');
    ...
    // トランザクションに署名して送信
    console.log(`${destinationAddresses.length}アドレスへの送金トランザクションを送信中...`);
    ...
    console.log(`✅ バッチ ${batchNumber} 成功! 所要時間: ${(endTime - startTime) / 1000}秒`);
...
async function main() {
  ...
  // 起動パラメータを確認
  const rpcUrl = process.argv[2] || CONFIG.rpcUrl;
  ...
  // 残高が十分かチェック
  if (balance < totalAmount + 0.01) { // 手数料のために0.01 SOL追加
    console.error(`エラー: 残高不足。必要: ${totalAmount + 0.01} SOL, 残高: ${balance} SOL`);
    process.exit(1);
  }
    ...
    if (result.success) {
      ...
      if (i < batches.length - 1) {
        ...
        const retryAnswer = await new Promise(resolve => {
          retryReadline.question('エラーが発生しました。続行しますか？ (y/n): ', resolve);
        });
